...
export default function ProfileScreen() {
  ...
  const [activeTab, setActiveTab] = useState<
    'PORTFOLIO' | 'CATALOG' | 'SAVED' | 'REFER' | 'ANALYTICS' | 'REELS' | 'LIKED'
  >('PORTFOLIO');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [
        ...
        supabase
          .rpc('get_business_performance_index', { target_user_id: uid })
          .then((res) => res.data),
      ...
      <View style={styles.analyticsContainer}>
        <View style={styles.scoreCard}>
          <View style={styles.scoreInfo}>
            ...
            <Text style={styles.scoreValue}>
              {perfIndex?.index_score ||
                (analytics?.stats?.engagement_rate
                  ? Math.round(Number(analytics.stats.engagement_rate) * 1.5)
                  : 92)}
          ...
          <View style={styles.scoreVisual}>
            <Ionicons
              name="ribbon"
              size={32}
              color={perfIndex?.status === 'ELITE' ? '#D4AF37' : '#00D084'}
            />
          </View>
        </View>
        ...
        <View style={styles.statGrid}>
          <View style={styles.statBoxModern}>
            ...
            <Text style={styles.statValModern}>
              {analytics?.stats?.total_views?.toLocaleString() || '0'}
            </Text>
            <Text style={styles.statLabelModern}>Market Reach</Text>
          </View>
          <View style={styles.statBoxModern}>
            <View style={styles.statIconWrap}>
              <Ionicons name="pulse" size={18} color="#00D084" />
            </View>
          ...
          <View style={styles.statBoxModern}>
            <View style={styles.statIconWrap}>
              <Ionicons name="briefcase" size={18} color="#00D084" />
            </View>
        ...
        <View style={styles.sectionHeaderRow}>
          ...
          <View style={styles.liveIndicator}>
            <Text style={styles.liveIndicatorText}>OPTIMIZED</Text>
          </View>
        </View>
        ...
        {strategyInsights.length > 0 ? (
          ...
          <View style={styles.insightCardModern}>
            <View style={styles.insightHeader}>
              <View style={styles.insightIcon}>
                <Ionicons name="bulb" size={20} color="#00D084" />
              </View>
              <Text style={styles.insightTitleModern}>Market Expansion</Text>
            </View>
            <Text style={styles.insightTextModern}>
              Our algorithms suggest your current content trajectory is ideal for High-Net-Worth
              acquisition. Continue scaling.
            </Text>
        ...
        <View style={styles.breakdownCard}>
          ...
          {[
            {
              label: 'Strategic Endorsements',
              value: analytics?.stats?.total_likes || 0,
              icon: 'heart-outline',
            },
            {
              label: 'Market Referrals',
              value: analytics?.stats?.total_reposts || 0,
              icon: 'repeat-outline',
            },
            {
              label: 'Intent-to-Purchase Saves',
              value: analytics?.stats?.total_shares || 0,
          ...
          <SafeLinearGradient colors={['#111', '#050505']} style={styles.feedbackGradient}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.feedbackSubtitle}>
                Direct line to our systems architecture team.
              </Text>
            </View>
  ...
  const renderHeader = () => (
    <View style={styles.header}>
      ...
      <View style={styles.navBar}>
        ...
        <View style={styles.navTitleContainer}>
          <Text style={styles.navTitle}>
            {profile?.business_name?.toUpperCase() || 'ENTERPRISE PROFILE'}
          </Text>
      ...
      {strategyInsights.length > 0 && (
        <TouchableOpacity style={styles.strategyPulse} onPress={() => setActiveTab('ANALYTICS')}>
      ...
      <View style={styles.profileInfoSection}>
            ...
            <StatItem
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { id: session?.user?.id, type: 'followers' },
                })
              }
            />
            <StatItem
              ...
              onPress={() =>
                router.push({
                  pathname: '/profile/follows',
                  params: { id: session?.user?.id, type: 'partners' },
                })
              }
            />
            <StatItem
              ...
              onPress={() =>
                router.push({
                  pathname: '/profile/follows',
                  params: { id: session?.user?.id, type: 'following' },
                })
              }
            />
          </View>
        ...
        <View style={styles.nameSection}>
          <View style={styles.displayNameRow}>
            ...
            >
              <Text
                style={[styles.tierText, perfIndex?.status === 'ELITE' && { color: '#D4AF37' }]}
              >
                {perfIndex?.status || 'PLATINUM'}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.actionRow}>
          ...
          >
            ...
            <Text style={styles.secondaryBtnText}>LIST PRODUCT</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.iconBtn} onPress={() => setActiveTab('ANALYTICS')}>
            <Ionicons
              name="analytics"
              size={20}
              color={activeTab === 'ANALYTICS' ? Colors.primary : '#fff'}
            />
          </TouchableOpacity>
        </View>
      </View>
    ...
    if (activeTab === 'CATALOG') {
      return (
        <TouchableOpacity
          ...
          onPress={() =>
            router.push({ pathname: '/profile/catalog', params: { id: session?.user?.id } })
          }
        >
            ...
            !loading ? (
              <View style={styles.emptyContainer}>
                <View style={styles.emptyIconCircle}>
                  <Ionicons
                    name={activeTab === 'SAVED' ? 'bookmark' : 'videocam'}
                    size={32}
                ...
                <Text style={styles.emptySubtitle}>
                  Your professional portfolio will appear here once synchronized.
                </Text>
                <TouchableOpacity
                  style={styles.emptyActionBtn}
                  onPress={() => router.push('/(tabs)/upload')}
                >
                  <Text style={styles.emptyActionText}>CREATE NEW ASSET</Text>
                </TouchableOpacity>
              </View>
            ) : null
          }
...
const styles = StyleSheet.create({
  ...
  navTitleContainer: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  navTitle: { color: '#fff', fontSize: 14, fontWeight: '900', letterSpacing: 2.5 },
  onlineBadge: {
    width: 8,
    height: 8,
    borderRadius: 4,
  ...
  strategyText: { color: '#00D084', fontSize: 11, fontWeight: '800', letterSpacing: 0.5 },
  iconCircle: {
    width: 44,
    height: 44,
    borderRadius: 15,
  ...
  profileInfoSection: { paddingHorizontal: 25, paddingBottom: 30 },
  avatarRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  ...
  statsContainer: { flex: 1, flexDirection: 'row', gap: 12, marginLeft: 25 },
  statItem: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
  ...
  tierText: { color: '#D4AF37', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 },
  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 8 },
  stars: { flexDirection: 'row', gap: 3 },
  ratingText: { color: '#FFD700', fontSize: 12, fontWeight: '800', opacity: 0.9 },
  handle: { color: '#00D084', fontSize: 14, fontWeight: '800', marginTop: 6, letterSpacing: 0.5 },
  bioText: { color: 'rgba(255,255,255,0.5)', fontSize: 15, lineHeight: 24, marginBottom: 25 },
  ...
  secondaryBtnText: { color: '#fff', fontSize: 12, fontWeight: '900', letterSpacing: 0.5 },
  iconBtn: {
    width: 52,
    height: 52,
    backgroundColor: 'rgba(255,255,255,0.05)',
