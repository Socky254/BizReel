...
import { AutonomousMaintenanceService, SecurityAnalysis } from '../../src/services/AutonomousMaintenanceService';
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.content}>
                <View style={styles.statusCard}>
                    ...
                    <Text style={styles.statusTime}>Last Audit: {new Date(analysis?.analytics.lastAudit || '').toLocaleTimeString()}</Text>
                </View>
                        ...
                        analysis?.suggestedFixes.map((fix) => (
                            <View key={fix.id} style={styles.fixCard}>
                                <View style={styles.fixHeader}>
                                    ...
                                    <View style={[styles.riskBadge, { backgroundColor: fix.riskLevel === 'low' ? 'rgba(0,208,132,0.1)' : 'rgba(255,59,48,0.1)' }]}>
                                        <Text style={[styles.riskText, { color: fix.riskLevel === 'low' ? Colors.primary : '#FF3B30' }]}>{fix.riskLevel.toUpperCase()}</Text>
                                    </View>
                                </View>
...
const styles = StyleSheet.create({
    ...
    loadingText: { color: 'rgba(255,255,255,0.4)', marginTop: 20, fontSize: 12, fontWeight: '800', letterSpacing: 2 },
    content: { padding: 20 },
    statusCard: { alignItems: 'center', padding: 30, backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 24, marginBottom: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.05)' },
    ...
    statBox: { flex: 1, backgroundColor: 'rgba(255,255,255,0.03)', padding: 15, borderRadius: 16, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.05)' },
    statLabel: { color: 'rgba(255,255,255,0.3)', fontSize: 9, fontWeight: '800', textTransform: 'uppercase', marginBottom: 6 },
    statValue: { color: '#fff', fontSize: 14, fontWeight: '900' },
    section: { marginBottom: 30 },
    sectionTitle: { color: '#fff', fontSize: 14, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 15 },
    concernItem: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10, backgroundColor: 'rgba(255,59,48,0.05)', padding: 12, borderRadius: 10 },
    concernText: { color: '#FF3B30', fontSize: 13, fontWeight: '600' },
    emptyText: { color: 'rgba(255,255,255,0.2)', fontSize: 14, fontStyle: 'italic' },
    fixCard: { backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 20, padding: 20, marginBottom: 15, borderWidth: 1, borderColor: 'rgba(255,255,255,0.05)' },
    fixHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
    ...
    fixBtn: { backgroundColor: 'rgba(255,255,255,0.05)', height: 44, borderRadius: 12, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
