...
export default function FollowsScreen() {
        ...
        <View style={styles.avatar}>
          {item.avatar_url ? (
            ...
            <View style={styles.avatarPlaceholder}>
              <Text style={styles.avatarText}>{item.business_name?.[0] || 'B'}</Text>
            </View>
          )}
        </View>
  ...
  const getHeaderTitle = () => {
    switch (type) {
      case 'followers':
        return 'Market (Followers)';
      case 'following':
        return 'Connections (Following)';
      case 'partners':
        return 'Partners (Mutual)';
      default:
        return 'Network';
    }
  };
  ...
  const getEmptyMessage = () => {
    switch (type) {
      case 'followers':
        return 'Your market is waiting for your first reel.';
      case 'following':
        return 'Start connecting with other businesses.';
      case 'partners':
        return 'Mutual partnerships appear here.';
      default:
